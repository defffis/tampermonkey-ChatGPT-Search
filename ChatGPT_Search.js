...
(function() {
  ...
  switch (lang) {
    ...
    case "ru-RU":
      config = {
        waitingResponse: "Ожидание ответа от ChatGPT...",
        login: "Пожалуйста, войдите в систему ",
        tooManyRequests: "Слишком много запросов за 1 час. Попробуйте еще раз позже.",
        checkClouflare: "Пожалуйста, пройдите проверку безопасности Cloudflare ",
        unknowError: "Упс, возможно, это ошибка, пожалуйста, проверьте или отправьте ",
        networkException: "Сетевая ошибка, пожалуйста, обновите страницу.",
        containerPosition: "Положение контейнера - Сбоку(1)/Сверху(0):",
  ...
  function containerShow(answer) {
    ...
    container2.querySelector("pre").textContent = answer.trim();
  }
  ...
  function getWebsite() {
    ...
    switch (location.hostname) {
      ...
      case "yandex.ru":
      case "yandex.by":
        return configRequestImmediately("yandex");
      case "www.deepl.com":
  ...
  function getQuestion() {
    switch (getWebsite().name) {
      ...
      case "yandex":
        return new URL(window.location.href).searchParams.get("text");
      default:
  ...
  function initUI() {
    ...
    function bingInjectContainer() {
      ...
      function yandexInjectContainer() {
      if (getPosition() === 1) {
        const container2 = getContainer();
        const siderbarContainer = document.getElementById("search-result-aside");
        siderbarContainer.prepend(container2);
      } else {
        GM_addStyle(".chat-gpt-container{max-width: 100%!important}");
        //GM_addStyle(".chat-gpt-container{width: 70vw}");
